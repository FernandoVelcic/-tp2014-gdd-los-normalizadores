...
namespace FrbaHotel.Database_Helper
{
    class FillerHelper
    {
        ...
        public static T buildObject<T>(SqlDataReader result)
        {
            ...
            String tableName = EntityManager.getEntityManager().getTableName(typeof(T));
            ...
            foreach (PropertyInfo property in propertyInfos)
            {
                ...
                else if (property.PropertyType.IsSubclassOf(typeof(ActiveRecord)))
                {
                    ...
                    String subTableName = EntityManager.getEntityManager().getTableName(property.PropertyType);

                    /* Como soluciono este copy past???? */
                    foreach (PropertyInfo subProperty in EntityManager.getEntityManager().getProperties(property.PropertyType))
                    {
